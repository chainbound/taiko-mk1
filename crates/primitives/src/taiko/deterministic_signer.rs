...
/// Errors that can occur when working with the deterministic signer
#[derive(Debug, thiserror::Error)]
#[allow(missing_docs)]
pub enum DeterministicSignerError {
    #[error("Invalid private key bytes")]
    InvalidPrivateKeyBytes,
    #[error("Cannot invert zero scalar")]
    CannotInvertZero,
    #[error("K value is too large and it resulted in an overflow: {0}")]
    KTooLarge(u128),
    #[error("Failed to sign hash using K = 1 and K = 2")]
    FailedToSignHash,
}
...
pub fn sign_hash_deterministic(
    secret_key: B256,
    hash: B256,
) -> Result<Signature, DeterministicSignerError> {
    // Try with k = 1
    ...
    Err(DeterministicSignerError::FailedToSignHash)
}
...
fn sign_hash_with_fixed_k(
    sk: B256,
    message: B256,
    k: u128,
) -> Result<[u8; 65], DeterministicSignerError> {
    ...
    if overflow {
        return Err(DeterministicSignerError::KTooLarge(k));
    }
    ...
    let Some(k_inv) = k_scalar.invert().into_option() else {
        return Err(DeterministicSignerError::CannotInvertZero);
    };
